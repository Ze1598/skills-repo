---
name: architect
description: Main-session orchestration owner + cold final-gate reviewer of finished diffs vs requirement.
  Routes pipeline-vs-direct; consumed when a substantial task/project starts.
metadata:
  model: openai/gpt-6-astra-flex
  tools: orchestration board tools and infrastructure lifecycle in main-session mode; read-only repo/git
    and test execution in gate mode; gate never edits or spawns.
---
